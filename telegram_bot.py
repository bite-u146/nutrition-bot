...
# Состояние ввода названия для избранного: user_id → unix-timestamp начала ожидания
_awaiting_fav_name: dict[int, float] = {}
_fav_name_nutrition: dict[int, dict] = {}
_FAV_NAME_TIMEOUT = 180  # секунд (3 минуты)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Перехватываем ввод названия для избранного (только если ожидание не истекло)
    if user_id in _awaiting_fav_name:
        elapsed = time.monotonic() - _awaiting_fav_name.pop(user_id)
        nutrition = _fav_name_nutrition.pop(user_id, None)
        if elapsed < _FAV_NAME_TIMEOUT:
            ...
            return
        # Таймаут истёк — обрабатываем сообщение как обычный запрос еды
...
async def handle_favorites_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Любая навигация, кроме самого «add_start», сбрасывает ожидание имени
    if action != "add_start":
        _awaiting_fav_name.pop(user_id, None)
        _fav_name_nutrition.pop(user_id, None)
    ...
    if action == "menu":
        ...
        if not nutrition:
            ...
            return
        _awaiting_fav_name[user_id] = time.monotonic()
        _fav_name_nutrition[user_id] = dict(nutrition)
        await query.edit_message_text(
